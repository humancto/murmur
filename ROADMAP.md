...
- [x] **settings-window** — SwiftUI Settings scene: vocabulary editor (multi-line text), cleanup toggle, audio cues toggle, max capture duration slider, hotkey rebinder. _Shipped in #10 (ef39f23). Single root `.onChange(of: settings)` for whole-struct persistence; multi-display unmount defense; hotkey rebinder via `KeyboardShortcuts.Recorder`._
