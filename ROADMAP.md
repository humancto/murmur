...
- [ ] **demoable-v0.1-vertical-slice** _(explicit grouping of items 8+9+10 — they're interdependent and shipping them as one PR gets the user a runnable demo without three serial review cycles)_:
  - **floating-hud-scaffold** — `NSPanel` at `.floating` level, click-through, `NSVisualEffectView` `.hudWindow` material, breathing-waveform SwiftUI view bound to a `@Published` amplitude. Reduced-motion respected.
  - **menu-bar-mic-indicator** — `NSStatusItem` with a state-driven icon: idle (mic outline), recording (filled mic + red dot). Bound to `AudioCapture.isCapturing`.
  - **end-to-end-v0.1** — Wire it together. Hotkey-down opens mic + shows HUD; hotkey-up runs VAD trim → WhisperKit transcribe → AXInjector or ClipboardInjector. App rebuilt as a proper AppKit app (NSApplicationMain) consuming `MurmurCore`. Acceptance: hold the bound key, speak, release, see the words appear in the focused field.
